...
case class Kotsu(kind: TileKind.Value, isMelded: Boolean) extends Mentsu {
    ...
    override def toString = {
        val prefix: String = isMelded match {
            case true => "/"
            case false => ""
        }
        val str: String = kind.isSuit match {
            case true => "(" + kind.rank + kind.rank + kind.rank + kind.category.char + ")"
...
case class Shuntsu(kind: TileKind.Value, isMelded: Boolean) extends Mentsu {
    ...
    override def toString = {
        ...
        val str: String = "(" + leftRank + middleRank + rightRank + char + ")"
        prefix + str
    }
}
...
case class Toitsu(kind: TileKind.Value) extends Part {
    ...
    override def toString = kind.isSuit match {
        case true => "<" + kind.rank + kind.rank + kind.category.char + ">"
...
case class Uki(kind: TileKind.Value) extends Part {
    ...
    override def toString = kind.isSuit match {
        case true => "|" + kind.rank + kind.category.char + "|"
        case false => "|" + kind + "|"
    }
}

case class Kantsu(kind: TileKind.Value, isMelded: Boolean) extends Mentsu {
    override def isKotsu = true

    override def isKantsu = true
    ...
    override def toString = {
        ...
        val str: String =
            kind.isSuit match {
                case true => "(" + kind.rank + kind.rank + kind.rank + kind.rank + kind.category.char + ")"
                case false => "(" + kind + kind + kind + kind + ")"
            }
        prefix + str
    }
}
